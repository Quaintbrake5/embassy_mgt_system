...
### FR-03: Passport Services
**User Story:** As a citizen, I want to apply for passport services (renewal, lost/stolen, emergency travel), so that I can travel legally.

#### Acceptance Criteria (EARS Format):
1. WHEN citizen submits passport application THEN system SHALL create application with status PENDING
2. WHEN officer reviews application THEN system SHALL allow status changes: APPROVE, REJECT, REQUEST_MORE_INFO
3. WHEN emergency travel certificate requested THEN system SHALL process with expedited workflow
4. WHEN lost/stolen passport reported THEN system SHALL flag in system and notify relevant authorities
5. WHEN passport issued THEN system SHALL generate document number and record in audit log

### FR-04: Civil Registry Services
**User Story:** As a citizen, I want to register vital events (birth, marriage, death) occurring abroad, so that they are legally recognized.

#### Acceptance Criteria (EARS Format):
1. WHEN birth registration submitted THEN system SHALL capture child details, parents' info, witness statements
2. WHEN marriage registration submitted THEN system SHALL capture both parties' details, witnesses, officer attestation
3. WHEN death registration submitted THEN system SHALL capture deceased details, cause, next of kin
4. WHEN certificate requested THEN system SHALL generate official certificate with digital seal
5. IF dual-registration detected THEN system SHALL flag for officer review
...
### FR-08: Emergency Services & Crisis Management
...
### FR-09: Diplomatic Administration & Logistics
...
### FR-10: Consular Revenue & Financial Ledger
...
### Core Entities (14)
1. **User** - System users with roles, status, authentication
2. **Role** - Named roles with permission sets
3. **Permission** - Granular resource:action permissions
4. **CitizenProfile** - Extended citizen information
5. **PassportApplication** - Passport service requests
6. **CivilRegistry** - Birth/marriage/death registrations
7. **VisaApplication** - Visa requests with documents
8. **Appointment** - Scheduled appointments with queue management
9. **DocumentLegalization** - Apostille/attestation requests
10. **EmergencyRegistration** - Crisis/evacuation tracking
11. **DiplomaticPouch** - Chain-of-custody tracking
12. **StaffClearance** - Diplomatic staff security clearances
13. **FinancialTransaction** - Fee collections and reconciliation
...
### Citizen Services
- POST /api/citizens/profile
- GET /api/citizens/profile
- PUT /api/citizens/profile
- POST /api/citizens/documents

### Passport Services
- POST /api/passport/applications
- GET /api/passport/applications
- GET /api/passport/applications/:id
- PUT /api/passport/applications/:id/status (officer)
- POST /api/passport/lost-stolen
- POST /api/passport/emergency-travel

### Civil Registry
- POST /api/civil-registry/birth
- POST /api/civil-registry/marriage
- POST /api/civil-registry/death
- GET /api/civil-registry/:id
- GET /api/civil-registry/certificate/:id
...
- POST /api/legalization/requests
- GET /api/legalization/requests
- GET /api/legalization/requests/:id
- PUT /api/legalization/requests/:id/process (officer)

### Emergency Services
- POST /api/emergency/registrations
- GET /api/emergency/registrations
...
| FR-02 Citizen Profile | P0 | Unit + Integration tests |
| FR-03 Passport Services | P1 | Integration + E2E tests |
| FR-04 Civil Registry | P1 | Integration + E2E tests |
...
| FR-08 Emergency | P1 | Integration + E2E tests |
| FR-09 Diplomatic Admin | P2 | Integration tests |
| FR-10 Financial | P1 | Integration tests |
...
- Phase 2 (Citizen Profile, Passport, Civil Registry): 3 weeks
- Phase 3 (Visa Processing, Appointments): 3 weeks

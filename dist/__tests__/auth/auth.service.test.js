...
describe('AuthService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new auth_service_1.AuthService(mockPrisma);
    });
    describe('register', () => {
        it('should register a new user successfully', async () => {
            mockPrisma.user.findUnique.mockResolvedValue(null);
            mockPrisma.user.create.mockResolvedValue((0, factories_1.createMockUser)({ status: 'PENDING' }));
        ...
        it('should reject duplicate email', async () => {
            mockPrisma.user.findUnique.mockResolvedValue((0, factories_1.createMockUser)());
    ...
    describe('login', () => {
        it('should login successfully with valid credentials', async () => {
            mockPrisma.user.findUnique.mockResolvedValue((0, factories_1.createMockUser)({ status: 'ACTIVE', role: null }));
        ...
        it('should reject suspended account', async () => {
            mockPrisma.user.findUnique.mockResolvedValue((0, factories_1.createMockUser)({ status: 'SUSPENDED', role: null }));
        ...
        it('should reject inactive account', async () => {
            mockPrisma.user.findUnique.mockResolvedValue((0, factories_1.createMockUser)({ status: 'INACTIVE', role: null }));
            await expect(service.login({ email: 'john@example.com', password: 'StrongP@ss1' })).rejects.toThrow('Account inactive');
        });
    });
    describe('refresh', () => {
        it('should refresh token successfully', async () => {
            mockPrisma.refreshToken.findUnique.mockResolvedValue((0, factories_1.createMockRefreshToken)({ isRevoked: false, expiresAt: new Date(Date.now() + 86400000), user: { ...(0, factories_1.createMockUser)({ status: 'ACTIVE', role: null }), role: null } }));

...
class EmbassyService {
    ...
    async create(dto, userId) {
        ...
        if (existing) {
            throw new exceptions_1.ConflictError('Embassy code already exists');
        }
    ...
    async update(embassyId, dto, userId) {
        const existing = await this.prisma.embassy.findUnique({
            where: { id: embassyId },
        });
        if (!existing) {
    ...
    async delete(embassyId, userId) {
        ...
        if (totalDependents > 0) {
            throw new exceptions_1.ConflictError(`Cannot delete embassy with ${totalDependents} active dependent records (departments, service requests, appointments, visa applications, emergency cases)`);
        }
    ...
    async createDepartment(embassyId, dto, userId) {
        const embassy = await this.prisma.embassy.findUnique({
            where: { id: embassyId },
        });
        if (!embassy) {
        ...
        if (existing) {
            throw new exceptions_1.ConflictError('Department slug already exists');
        }
    ...
    async findDepartments(embassyId) {
    ...
    async updateDepartment(departmentId, dto, userId) {
        const existing = await this.prisma.department.findUnique({
            where: { id: departmentId },
        });
        if (!existing) {
            throw new exceptions_1.NotFoundError('Department not found');
        }

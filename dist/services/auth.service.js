...
class AuthService {
    ...
    async refresh(refreshToken) {
        ...
        const newAccessToken = (0, jwt_utilities_1.signAccessToken)({ userId: user.userid, email: user.email, roleId: user.roleId ?? undefined });
        const newRefreshToken = (0, jwt_utilities_1.signRefreshToken)({ userId: user.userid });
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + 7);
        await this.prisma.refreshToken.create({
            data: {

...
class AppointmentService {
    ...
    async getAvailableSlots(embassyId, date) {
    ...
    async book(dto, userId) {
        ...
        if (serviceRequest.userId !== userId) {
            throw new exceptions_1.ValidationError('Service request does not belong to this user');
        }
        if (serviceRequest.embassyId !== dto.embassyId) {
            throw new exceptions_1.ValidationError('Service request embassy does not match appointment embassy');
        }
        ...
        const appointment = await this.prisma.$transaction(async (tx) => {
            ...
            if (existingSlot) {
                throw new exceptions_1.ConflictError('This time slot is already booked');
            }
            const qrCode = (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase();
    ...
    async findMyAppointments(userId, page, limit) {
    ...
    async cancel(appointmentId, userId) {
        ...
        if (!appointment) {
            throw new exceptions_1.NotFoundError('Appointment not found');
        }
        if (appointment.status !== 'BOOKED') {
            throw new exceptions_1.ValidationError(`Cannot cancel appointment with status ${appointment.status}. Only BOOKED appointments can be cancelled.`);
        }
    ...
    async checkIn(appointmentId, otp) {
        const appointment = await this.prisma.appointment.findUnique({
            where: { id: appointmentId },
            include: AppointmentService.APPOINTMENT_INCLUDE,
        });
        ...
        if (appointment.status !== 'BOOKED') {
            throw new exceptions_1.ValidationError(`Cannot check in appointment with status ${appointment.status}. Only BOOKED appointments can be checked in.`);
        }
        const isValid = await this.otpService.verifyOtp(appointmentId, otp);
        if (!isValid) {
            throw new exceptions_1.ValidationError('Invalid or expired OTP');
        }
        const updated = await this.prisma.appointment.update({
            where: { id: appointmentId },
    ...
    async getQueue(embassyId, page = 1, limit = 50) {
    ...
    async callNext(embassyId) {
        ...
        if (!nextAppointment) {
            throw new exceptions_1.NotFoundError('No checked-in appointments in queue');
        }
        const updated = await this.prisma.appointment.update({
    ...
    async complete(appointmentId) {
        ...
        if (appointment.status !== 'IN_PROGRESS') {
            throw new exceptions_1.ValidationError(`Cannot complete appointment with status ${appointment.status}. Only IN_PROGRESS appointments can be completed.`);
        }
        const updated = await this.prisma.appointment.update({
            where: { id: appointmentId },
    ...
    async markNoShow(appointmentId) {
        ...
        if (appointment.status !== 'BOOKED') {
            throw new exceptions_1.ValidationError(`Cannot mark no-show for appointment with status ${appointment.status}. Only BOOKED appointments can be marked no-show.`);
        }
        const now = new Date();
        if (now < appointment.slotDate) {
            throw new exceptions_1.ValidationError('Cannot mark no-show before the appointment slot date');
        }
        const updated = await this.prisma.appointment.update({
            where: { id: appointmentId },
            data: { status: 'NO_SHOW' },
...
AppointmentService.APPOINTMENT_INCLUDE = {

...
import { CreateVisaDecisionDto, VisaDecisionResponseDto } from '../dto/visa-decision.dto';
...
export interface IVisaDecisionService {
  createDecision(applicationId: string, dto: CreateVisaDecisionDto, officerId: string): Promise<VisaDecisionResponseDto>;
  getDecision(applicationId: string): Promise<VisaDecisionResponseDto>;
  getDecisionsByOfficer(officerId: string, page?: number, limit?: number): Promise<{ data: VisaDecisionResponseDto[]; meta: { total: number; page: number; limit: number; totalPages: number } }>;
}

export class VisaDecisionService implements IVisaDecisionService {
  private prisma: PrismaClient;

  private static readonly DECISION_INCLUDE = {
  ...
  async createDecision(applicationId: string, dto: CreateVisaDecisionDto, officerId: string): Promise<VisaDecisionResponseDto> {
    ...
    if (!ALLOWED_TRANSITION_STATUSES.includes(application.status)) {
      throw new ValidationError(
        `Cannot make decision on application in status ${application.status}. Allowed: ${ALLOWED_TRANSITION_STATUSES.join(', ')}`
      );
    }

    if (dto.decision === 'ESCALATE_TO_HQ' && !dto.secondaryOfficerId) {
      throw new ValidationError('Secondary officer ID is required when escalating to HQ');
    }
    ...
    if (dto.secondaryOfficerId) {
      ...
      if (!secondaryOfficer) {
        throw new NotFoundError('Secondary officer not found');
      }
    }

    const targetStatus = DECISION_STATUS_MAP[dto.decision];
    if (!targetStatus) {
      throw new ValidationError(`Invalid decision type: ${dto.decision}`);
    }
  ...
  async getDecision(applicationId: string): Promise<VisaDecisionResponseDto> {
    ...
    if (!decision) {
      throw new NotFoundError('Visa decision not found for this application');
    }

    return this.toResponse(decision);
  }

  async getDecisionsByOfficer(officerId: string, page = 1, limit = 10): Promise<{ data: VisaDecisionResponseDto[]; meta: { total: number; page: number; limit: number; totalPages: number } }> {
  ...
  private toResponse(decision: any): VisaDecisionResponseDto {
    return {
      id: decision.id,
